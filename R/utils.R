...
#' @param CR A numeric column vector of length `N * T` to replace the k-th column in each block of the matrix.
...
#' k <- 2
#' M_updated <- replace_kth_column_block_diagonal_fast(M, CR, T, K, N, k)
#'
...
#' Initialize a Dense Covariance Matrix Using Cholesky Decomposition
#'
#' This function generates a random \eqn{\( p \times p \)} symmetric positive-definite
#' covariance matrix using the Cholesky decomposition method. The matrix is
#' generated by taking \( A^T A \), where \( A \) is a random matrix, ensuring that
#' the resulting matrix is symmetric and positive-definite.
#'
#' @param p An integer specifying the dimension of the covariance matrix (i.e.,
#' the number of rows/columns).
#' @param scale A numeric value to scale the resulting covariance matrix. This is
#' used to control the magnitude of the variances. Default is 1.
#'
#' @return A \eqn{\( p \times p \)} symmetric positive-definite covariance matrix.
#'
#' @examples
#' set.seed(123)
#' p <- 3  # Dimension of the covariance matrix
#' cov_matrix <- initialize_dense_covariance(p)
#' print(cov_matrix)
#'
#' @export
initialize_dense_covariance <- function(p, scale = 1) {
  ...
  # Scale the matrix to control the magnitude of the variance
  Sigma <- Sigma * scale
